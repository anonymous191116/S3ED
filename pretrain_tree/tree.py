...
class VPtree(nn.Module):
    ...
    def forward(self, att_feats):
        ...
        # print(sub1.size())
        sub1 = self.fc1_entity(self.avgpool(sub1).squeeze(2).squeeze(2))  # [batch,64]
        obj1 = self.fc1_entity(self.avgpool(obj1).squeeze(2).squeeze(2))
        sub2 = self.fc1_entity(self.avgpool(sub2).squeeze(2).squeeze(2))
        obj2 = self.fc1_entity(self.avgpool(obj2).squeeze(2).squeeze(2))
        re12 = self.fc1_relation(self.avgpool(re12).squeeze(2).squeeze(2))
        re34 = self.fc1_relation(self.avgpool(re34).squeeze(2).squeeze(2))
        reroot = self.fc1_relation(self.avgpool(reroot).squeeze(2).squeeze(2))

        return  sub1,re12,obj1,reroot,sub2,re34 ,obj2
    #
...
# class VPtree_ABB(nn.Module):
#     def __init__(self,ENTITY_NUM,RELATION_NUM):
#         super(VPtree_ABB, self).__init__()
#         self.entity_num=ENTITY_NUM
#         self.relation_num = RELATION_NUM
#         self.entityconv1 = nn.Conv2d(2048,64,3)
#         self.entityconv2 = nn.Conv2d(2048,64,3)
#         self.entityconv3 = nn.Conv2d(2048,64,3)
#         self.entityconv4 = nn.Conv2d(2048,64, 3)
#         self.relationconv12 = nn.Conv2d(2048,64, 3)
#         self.relationconv34 = nn.Conv2d(2048,64, 3)
#         self.relationconvroot = nn.Conv2d(2048,64, 3)
#         self.bn = nn.BatchNorm2d(64)
#         self.relu = nn.ReLU(inplace=True)
#
#         self.avgpool = nn.AdaptiveAvgPool2d(1)
...
#         sub2 = self.relu(self.bn(self.entityconv3(x)))
#         obj2 = self.relu(self.bn(self.entityconv4(x)))
#         re12 = self.relu(self.bn(self.relationconv12(x)))
#         re34 = self.relu(self.bn(self.relationconv34(x)))
#         reroot = self.relu(self.bn(self.relationconvroot(x)))
#         # print(sub1.size())
