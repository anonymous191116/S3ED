...
import numpy as np
###############################################################################
# Adding Semantic-Guided Embedding Tree model at ResnetGenerator()
# TODO 
# adding:
# 1.UNET++
# 2.EfficientNet-Encoder with Tree model
###############################################################################
...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, gpu_ids=[], use_parallel = True, learn_residual = False):
	...
	elif which_model_netG == 'resnet_6blocks_tree':
...
class ResnetGenerator(nn.Module):
	def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[], use_parallel = True, 
					learn_residual = False, padding_type='reflect', tree_node_dim = 64, tree_embeding_dim = [256,128]):
		assert(n_blocks >= 0)
		...
		# '''
		# add tree here to maintain featuremap value distribution
		# '''
		model1 =[]
		...
		##############################################################################
		# Semantic-Guided Embedding 
		##############################################################################
		self.tree_unsample = nn.Sequential(
			*[nn.ConvTranspose2d(7*tree_node_dim, tree_embeding_dim[0],kernel_size=3, stride=2,padding=1,bias=use_bias),
			 norm_layer(tree_embeding_dim[0]),
			 nn.ReLU(True),

			 nn.ConvTranspose2d(tree_embeding_dim[0], tree_embeding_dim[1],kernel_size=2, stride=3, padding=2,bias=use_bias), 
			 norm_layer(tree_embeding_dim[1]),
			 nn.ReLU(True)]
		)

		mult = 2**(n_downsampling)
		self.semantic_fusion = nn.Sequential(*[
			nn.Conv2d(tree_embeding_dim[1] + (ngf * mult) ,(ngf * mult), 1),
			norm_layer(ngf * mult),
			nn.ReLU(True)])
	...
	def forward(self, input,tree_att):
		if self.gpu_ids and isinstance(input.data, torch.cuda.FloatTensor) and self.use_parallel:
			output = nn.parallel.data_parallel(self.model, input, self.gpu_ids)
		else:
			output1 = self.model(input)
			output2 = self.semantic_fusion(torch.cat((output1,self.tree_unsample(tree_att)),1))#add tree
			output =self.model1(output2)
