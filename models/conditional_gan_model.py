...
class ConditionalGAN(BaseModel):
	...
	def backward_G(self):
		...
		self.loss_G = self.loss_G_GAN + self.loss_G_Content + self.loss_Tree
	...
	def get_current_errors(self):
		return OrderedDict([('G_GAN', self.loss_G_GAN.data),
							('G_L1', self.loss_G_Content.data),
							('D_real+fake', self.loss_D.data)
							])
	...
	def save(self, label):
		...
		self.save_network(self.netTree,'T',label, self.gpu_ids)
